...
export async function GET(req: NextRequest) {
  ...
  const result = await db
    ...
    .orderBy(goals.category, goals.sortOrder, goals.createdAt);
...
export async function POST(req: NextRequest) {
  ...
  if (!title || !category || !scope || !year) {
    return NextResponse.json({ error: "title, category, scope, year required" }, { status: 400 });
  }
