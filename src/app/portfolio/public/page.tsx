      ...
      <div className="max-w-3xl mx-auto px-8 py-16">
        ...
        {featured.length > 0 && (
          <section className="mb-12">
            ...
            <div className="space-y-4">{featured.map((p) => <ProjectCard key={p.id} project={p} large />)}</div>
          </section>
        )}
        {projectItems.filter((p) => !p.featured).length > 0 && (
          <section className="mb-12">
            ...
            <div className="space-y-3">{projectItems.filter((p) => !p.featured).map((p) => <ProjectCard key={p.id} project={p} />)}</div>
          </section>
        )}
        {postItems.filter((p) => !p.featured).length > 0 && (
          <section>
            ...
            <div className="space-y-3">{postItems.filter((p) => !p.featured).map((p) => <ProjectCard key={p.id} project={p} />)}</div>
          </section>
        )}
...
function ProjectCard({ project, large = false }: { project: any; large?: boolean }) {
  ...
  return (
    <Wrapper {...wrapperProps} className={`group block p-5 rounded-2xl border border-stone-200/60 bg-white/50 hover:bg-white/80 hover:border-stone-300 transition-all ${large ? "p-6" : ""}`}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <p className={`font-semibold text-stone-800 ${large ? "text-xl" : "text-base"}`}>{project.title}</p>
          {project.description && <p className="text-stone-500 text-sm mt-1.5 leading-relaxed line-clamp-2">{project.description}</p>}
          {project.tags?.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-3">
              {project.tags.map((tag: string) => <span key={tag} className="text-[11px] px-2 py-0.5 bg-stone-100 text-stone-500 rounded-full">{tag}</span>)}
            </div>
          )}
        </div>
