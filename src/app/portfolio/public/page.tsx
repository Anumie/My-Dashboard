  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <div className="max-w-3xl mx-auto px-8 py-16">
        {/* Hero */}
        <div className="mb-16">
          <h1 className="text-4xl font-semibold text-stone-800 tracking-tight mb-3">
            Work & Writing
          </h1>
          <p className="text-stone-500 text-lg">
            A collection of projects and posts.
          </p>
        </div>
        ...
        {postItems.filter((p) => !p.featured).length > 0 && (
          <section>
            ...
            <div className="space-y-3">
              {postItems.filter((p) => !p.featured).map((project) => (
                <ProjectCard key={project.id} project={project} />
              ))}
            </div>
          </section>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
          ...
          {project.tags?.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-3">
              {project.tags.map((tag: string) => (
                <span key={tag} className="text-[11px] px-2 py-0.5 bg-stone-100 text-stone-500 rounded-full">
                  {tag}
                </span>
              ))}
            </div>
        ...
        {project.link && (
          <ExternalLink className="w-4 h-4 text-stone-300 group-hover:text-stone-500 transition-colors flex-shrink-0 mt-0.5" />
        )}
      </div>
    </Wrapper>
  );
